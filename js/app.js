(() => {
    ...
    (() => {
        ...
        function menuInit() {
            let body = document.querySelector("body");
            const pageHat = document.querySelector(".header");
        ...
        da.init();
        let pageElement = document.getElementsByTagName("section")[2];
        let elementPosition = pageElement.offsetTop;
        const pageHat = document.querySelector(".header");
        const pageNav = document.querySelector(".navigation");
        window.addEventListener("scroll", script_addClass);
        function script_addClass() {
            let windowY = window.scrollY;
            if (windowY >= elementPosition || windowY > 800) {
                pageHat.classList.add("header-visible");
                pageNav.classList.add("navigation-show");
            } else {
                pageHat.classList.remove("header-visible");
                pageNav.classList.remove("navigation-show");
            }
        }
        window.addEventListener("click", showHint);
        function showHint(e) {
            const showHintButtons = document.querySelectorAll(".hint__button");
            const hintBlocks = document.querySelectorAll(".hint");
            if (showHintButtons && e.target.closest(".hint__button")) hintBlocks.forEach((hintBlock => {
                hintBlock.classList.toggle("hint-active");
            }));
        }
